...
fig
ggsave(fig, filename = "~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig 4A.tiff",width = 3,height = 4)
...
fig
ggsave(fig, filename = "~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig s4.tiff",width = 3,height = 4)
ggsave(fig, filename = "~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig s4.pdf",width = 3,height = 4)
```
...
tiff("~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig 4b.tiff",width = 4,height = 3.5,units = "in",res = 300)
# par(mar=c(1,1,1,1))
# par("mar") 
plot_ave_roc(annotated_pred_df = tmp  , addlegend = F)
dev.off() 

pdf("~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig 4b.pdf",width = 4,height = 3.5)
plot_ave_roc(CTRP_binary_PPIold_data%>% 
  mutate(imptype=recode(imptype, "ConExp-Sig"= "exp-sig" , "ConSen-Sig"= "ess-sig")) , addlegend = F)
dev.off() 

tiff("~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig 4c.tiff",width = 4,height = 3.5,units = "in",res = 300)
plot_ave_roc(GDSC_binary_PPIold_data%>% 
  mutate(imptype=recode(imptype, "ConExp-Sig"= "exp-sig" , "ConSen-Sig"= "ess-sig")) , title = "GDSC", addlegend = T)
dev.off() 

pdf("~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res/fig 4c.pdf",width = 4,height = 3.5)
plot_ave_roc(GDSC_binary_PPIold_data%>% 
  mutate(imptype=recode(imptype, "ConExp-Sig"= "exp-sig" , "ConSen-Sig"= "ess-sig")) , title = "GDSC", addlegend = T)
dev.off() 
...
## 4
Is there any genes universally ranked higher?
What is the expectation of MDR1 and ABCB1?
I suppose they ranked highly irrespecive of drug identity.
WEll not really..

Lets further confirm which gene are more frequently ranked in the top or bottom
in ess-sig?
note stick to valid signatures.

# Specificity. MCL1 
```{r}

generank_ctrp <- feature_imp_ridge_ctrp_comb1 %>%
  filter(drug %in% ctrp_allvalid_list) %>%
  # filter(drug %in% ctrp_target_binary$drug) %>%
  pivot_longer(-drug,names_to="gene", values_to="imp") %>%
  group_by(drug) %>%
  mutate(gene_rank= rank(-imp,ties.method = "min")) %>%
  ungroup() %>%
  group_by(gene) %>%
  summarize(medianrank= median(gene_rank),
            meanrank= mean(gene_rank),
            percent1rank= quantile(gene_rank, 0.01,type = 1),
            percent5rank= quantile(gene_rank, 0.05,type = 1),
            percent10rank= quantile(gene_rank, 0.10,type = 1),
            percent25rank= quantile(gene_rank, 0.25,type = 1),
            percent75rank= quantile(gene_rank, 0.75,type = 1),
            percent90rank= quantile(gene_rank, 0.90,type = 1),
            percent95rank= quantile(gene_rank, 0.95,type = 1),
            percent99rank= quantile(gene_rank, 0.99,type = 1),
            Numdrugranked50= sum(gene_rank< 51),
            Numdrugranked100= sum(gene_rank< 101),
            Numdrugrankedbottom50= sum(gene_rank> 10573),
            Numdrugrankedbottom100= sum(gene_rank>10523)
            ) %>% 
  ungroup()

target_freq_ctrp <- ctrp_target_binary %>% 
  filter(drug %in% ctrp_allvalid_list) %>% 
  count(target) %>% 
  arrange(desc(n))

MCL1_ctrp <- feature_imp_ridge_ctrp_comb1 %>%
  filter(drug %in% ctrp_allvalid_list) %>%
  # filter(drug %in% ctrp_target_binary$drug) %>%
  pivot_longer(-drug,names_to="gene", values_to="imp") %>%
  group_by(drug) %>%
  ...
  filter(gene == "MCL1") %>% 
  inner_join(
    y= ctrp_data %>% select(broad_cpd_id, target_or_activity_of_compound,cpd_name,target),
    by=c("drug" = "broad_cpd_id")
    ) %>% 
  mutate(BCL_pathway= str_detect(target_or_activity_of_compound, pattern = "BCL|MCL1")) %>% 
  mutate(target= map_chr(target, function(x){paste(x$gene_symbol_of_protein_target,collapse = ",")}))

setwd("~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res")
write_csv(MCL1_ctrp,"MCL1_ctrp.csv")
write_csv(generank_ctrp,"generank_ctrp.csv")
write_csv(target_freq_ctrp,"target_freq_ctrp.csv")

  

generank_gdsc <- feature_imp_ridge_gdsc_comb1 %>%
  filter(drug %in% gdsc_allvalid_list) %>%
  pivot_longer(-drug,names_to="gene", values_to="imp") %>%
  group_by(drug) %>%
  mutate(gene_rank= rank(-imp,ties.method = "min")) %>%
...
# what are the most commonly affected pathway?
gdsc_data %>% filter(DRUG_ID %in% gdsc_allvalid_list) %>% 
  count(PATHWAY_NAME) %>% 
  arrange(desc(n))

target_freq_gdsc <- gdsc_target_binary %>% 
  filter(drug %in% gdsc_allvalid_list) %>% 
  count(target) %>% 
  arrange(desc(n))
setwd("~/cluster_wrk/drug_moa/unsupervised_target_exploration/fig_res")
write_csv(generank_gdsc,"generank_gdsc.csv")
write_csv(target_freq_gdsc,"target_freq_gdsc.csv")

#on the other hand, ABCB1 is not a gene of interest??
