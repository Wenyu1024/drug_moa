library(tidyverse)
setwd("/home/cloud-user/cluster_scratch/depmap_21q1/")
list.files()
...
save.image("/home/cloud-user/cluster_scratch/ces_io/ces_prepare_21q1_intermediate.RData")

# The following calculation were put on a server
# df_long_scaled_noarray <- 
#   wide_to_long1(ceres, "ceres",cells1) %>% 
#   inner_join(wide_to_long1(demeter2, "demeter2",cells1),by=c("DepMap_ID", "gene")) %>% 
#   left_join(wide_to_long1(exp_seq, "exp_seq",cells1),by=c("DepMap_ID", "gene")) %>% 
#   left_join(wide_to_long1(mut, "mut",cells1) ,by=c("DepMap_ID", "gene")) %>% 
#   left_join(wide_to_long1(cn, "cn",cells1) ,by=c("DepMap_ID", "gene")) %>% 
#   drop_na() %>% 
#   group_by(DepMap_ID) %>% 
