library(tidyverse)
setwd("/home/cloud-user/cluster_scratch/depmap_20q4/")
list.files()
cell_info <- read_csv("sample_info.csv")
ceres_broad<- read_csv("ceres.csv")
cn <- read_csv("cn.csv")
exp_seq <- read_csv("seq.csv")
mut <- data.table::fread(file = "mutations.csv")
...
df_long <- wide_to_long(ceres, "ceres") %>% 
  inner_join(wide_to_long(demeter2, "demeter2"),by=c("DepMap_ID", "gene")) %>% 
  left_join(wide_to_long(exp_array, "exp_array"),by=c("DepMap_ID", "gene")) %>% 
  left_join(wide_to_long(exp_seq, "exp_seq"),by=c("DepMap_ID", "gene")) %>% 
  left_join(wide_to_long(mut, "mut") ,by=c("DepMap_ID", "gene")) %>% 
  left_join(wide_to_long(cn, "cn") ,by=c("DepMap_ID", "gene"))
...
wide_to_long1 <- function(df, value){
  df %>% 
    filter(DepMap_ID %in% cells2) %>% 
    gather(key= "gene", value= !!value, 2:ncol(df)) %>% 
    filter(gene %in% gene_ess)
...
df_long_scaled_noarray <- 
  wide_to_long1(ceres, "ceres") %>% 
  inner_join(wide_to_long1(demeter2, "demeter2"),by=c("DepMap_ID", "gene")) %>% 
left_join(wide_to_long1(exp_seq, "exp_seq"),by=c("DepMap_ID", "gene")) %>% 
  left_join(wide_to_long1(mut, "mut") ,by=c("DepMap_ID", "gene")) %>% 
  left_join(wide_to_long1(cn, "cn") ,by=c("DepMap_ID", "gene")) %>% 
  drop_na() %>% 
  group_by(DepMap_ID) %>% 
  mutate_at(.vars = 3:7,.funs = scale) %>% 
  ungroup()

setwd("~/cluster_scratch/ces_io")
write_csv(df_long_scaled, "ces_input_20q4.csv")
write_csv(df_long_scaled_noarray,"ces_input_noarray_20q4.csv")
write_csv(ceres, "ceres_20q4.csv")
write_csv(demeter2, "demeter2_20q4.csv")
save.image("ces_prepare_20q4.RData")

## what about impute each matrix first before fitting the ces.