...
prism_nononcology_df <- read_csv("~/cluster_wrk/drug_moa/predicting_prism/prism_nononcology_df.csv")  
...
PRISM_63inchikey <- read_csv("~/cluster_scratch/prism/drug_repurposing_hub_sample.csv") %>%
  select(broad_id,pert_iname,smiles,InChIKey,pubchem_cid) %>%
  separate(col = broad_id,into=c("BROAD_ID","id",NA),sep="-") %>%
  unite("BROAD_ID",BROAD_ID:id, sep= "-",remove = T) %>%
  distinct() %>%
  ...
  distinct()
tmp1 <- inner_join(PRISM_63inchikey,
                  FIMM_compounds_Annotation_ZIA,
                  by=c("InChIKey"= "standaryInchiKet")) %>%
  inner_join(prism_data %>% select(-target, -sensitivity), by= c("BROAD_ID"))
