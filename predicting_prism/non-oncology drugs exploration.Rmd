...
# drug_of_clustering_interest <- prism_268_all %>% 
#   filter(drug %in% prism_good_fitted_drugs_drhannotated ) %>% 
#   # filter((drug %in% cancer_drug_category$drug)|(Drug_category %in% "noncancer")) %>% 
#   arrange(drug) %>% 
#   pull(drug)
# 
# tmp <- feature_imp_ridge_prism_comb1 %>% 
#   filter(drug %in% drug_of_clustering_interest ) %>% 
...
#   rename(tSNE1="V1",
#          tSNE2="V2") %>% 
#   bind_cols(drug=drug_of_clustering_interest, cost= tmp$costs  ) %>% 
...
#   geom_point(data = tmp1 %>% 
#                filter( Drug_category !="noncancer") %>%
#                filter(drug %in% cancer_drug_category),
...
# tmp <- dimreduction_df %>% 
#   filter((moa %in% cancer_drug_category$moa)|(Drug_category=="noncancer"))
  # filter(Drug_category !="noncancer") %>% 
  # mutate(anno= moa) %>%
  # ungroup() %>% 
  # why this is not working!!!!!!!!!!!?????
  # mutate(anno= case_when(  
  #   (Drug_category=="noncancer") ~ NA,
  #   TRUE ~ anno)) %>% 
