...
res <- map_dfr(c(1:1453), .f = function(x){
  
    drug <- paste("~/cluster_scratch/forward_modelling/prism_auc_spearman_nofiltering/drug_", x, ".RData", sep = "")
  load(drug)
...
```{r}
# tmp <- str_subset(string = list.files(path =  "/home/cloud-user/cluster_scratch/forward_modelling/prism_auc_spearman_filtering/"),pattern = "drug_" )
# tmp  <- str_split(tmp,pattern = "_",n = 2,simplify = T)
# tmp <-  str_split(tmp[,2],pattern = ".RData",n = 2,simplify = T)
# error_idx <- setdiff(x= 1:1453, y = as.integer(tmp[,1]))
...
res2 %>%
  ggplot(aes(x=obs_num_auc,y= ces)) +
  geom_point(size=0.5)+
  theme_classic()+
  xlab("Sample Size")+
...
res2 %>% 
  ggplot(aes(x=obs_num_auc,y= ces_variance)) +
  geom_point(size=0.5)+
  theme_classic()+
  xlab("Sample Size")+
...
lets now check what is the prediction accuracy of filtering

```{r}

res_variablefiltered <- map_dfr(c(1:1453), .f = function(x){
  
    drug <- paste("~/cluster_scratch/forward_modelling/prism_auc_spearman_filtering/drug_", x, ".RData", sep = "")
  load(drug)
  df <- bind_cols( ces= ces1_perf, drug= x)
...
```{r}
res_variablefiltered1 <- res_variablefiltered %>% 
  group_by(drug) %>% 
  summarise_all(.funs = median) 

res_variablefiltered2 <- res_variablefiltered1 %>% 
  select(-drug) %>% 
  bind_cols(prism_data) %>% 
  mutate(sample_size= map_int(.x=sensitivity, 
                              .f=function(x){sum(x %>% pull(depmap_id) %in% ces1_478$DepMap_ID)})) %>%
...
# res_variablefiltered1 %>%   
#   select(-drug)%>%   
#   pivot_longer(cols = 1:4,names_to= "predictor", values_to= "spearman_cor") %>% 
#   group_by(predictor) %>% 
#   summarise(mean_cor= mean(spearman_cor), median_cor= median(spearman_cor))

summary(res_variablefiltered2$ces)

res_variablefiltered2 %>% arrange(desc(ces))
```


```{r,fig.width=2.5, fig.height=2.5}
res_variablefiltered2 %>%
  ggplot(aes(x=obs_num_auc,y= ces)) +
  geom_point(size=0.5)+
  theme_classic()+
  xlab("Sample Size")+
...
compare
```{r}
res_ridge_spearman <- inner_join(
  x = res2 %>% rename(no_filtering= ces),
  y= res_variablefiltered2 %>% 
    select(ces,broad_id) %>% 
    rename(filtering=ces)
  )

t.test(res_ridge_spearman$no_filtering, res_ridge_spearman$filtering,paired = T)

cor.test(res_ridge_spearman$no_filtering, res_ridge_spearman$filtering)

res_ridge_spearman %>% 
  ggplot(aes(x=no_filtering, y=filtering))+
  geom_point()+
  theme_classic()
```
