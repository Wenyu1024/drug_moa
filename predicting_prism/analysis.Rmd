...
table(prism_sc_drug %>%  filter(drug_category=="noncancer") %>% pull(phase))

tmp <- prism_sc_drug %>% 
  filter(target=="NA")

tmp1 <- tmp %>% filter(moa=="NA")
```
...
targetinfo_drug_by_category <- prism_sc_drug %>% 
  ...
  # mutate(drug_phase= fct_relevel(drug_phase, levels= c("Approved", "Investigational") )) %>%
...
fig <- targetinfo_drug_by_category %>% 
  ...
  ylab("Number of drugs lacking target genes")+
...
write_csv(targetinfo_drug_by_category, "targetinfo_drug_by_categor.csv")
...
# fig <- targetinfo_drug_by_category %>% 
#   mutate(avaliable= total_count-na_count) %>%
#   rename(`not avaliable`= na_count) %>% 
#   # select(-total_count) %>% 
#   pivot_longer(one_of("not avaliable", "avaliable"), names_to= "tar_aval", values_to="count" ) %>% 
#   ggplot(aes(x=drug_category, y=count, fill=drug_category, color=tar_aval)) +
...
fig <- sen_mean_var %>% 
  ...
  geom_smooth(method = "lm", se = FALSE)+
  theme_classic()+
  ylab("Prediction accuracy (Spearman coef)")+
...
ggsave(fig,filename = "~/cluster_wrk/drug_moa/predicting_prism/fig_res/fig5c.png",width = 3,height = 7,units = "in",dpi = 500)
ggsave(fig,filename = "~/cluster_wrk/drug_moa/predicting_prism/fig_res/fig5c.pdf",device = "pdf",width = 7,height = 3)
```
...
# df <- bind_rows(
#   acc_df_prism_binary_fold_cv_good200 %>% 
#     mutate(fitting_quality= "Top"),
#   acc_df_prism_binary_fold_cv_bad200 %>% 
#     mutate(fitting_quality= "Bottom"),
#   acc_df_prism_binary_fold_cv %>% 
#     mutate(fitting_quality= "All")   
#   ) %>% 
#   mutate(fitting_quality= 
#            fct_relevel(fitting_quality, c("All", "Top", "Bottom"))) %>%
...
#     .x = data, 
#     .f = function(data){
#       res=DescTools::DunnettTest(data$acc, g= data$method)
...
```{r}
#How about not comparing to other signatures
  # my_comparison1 <- list(c("Top", "All"), c("Top", "Bottom"), c("All", "Bottom"))
  # df %>% 
  #   filter(method %in% c("Ess-sig")) %>% 
  #   ggplot(aes(y=acc, x= fitting_quality)) + 
  #   geom_violin()+
  #   ggpubr::stat_compare_means(
  #     aes(group=fitting_quality),label = "p.signif",
  #     method = "t.test",comparisons = my_comparison1,vjust = 0.4)+
  #   stat_summary(fun.data = "mean_cl_boot", geom = "pointrange",
  #            colour = "black",position = position_dodge(width = 0.9),size=0.2)+
  #   ggsci::scale_fill_npg()+
  #   ggsci::scale_color_npg()+
...
fig <- unsupervised_df %>% 
  ...
  guides(fill=guide_legend(nrow = 1))+
  theme_classic()+
  ylab(label = "Unsupervised target prediction (AUC)")+
...
#                filter(drug %in% prism_good_drug_list_200),
#              title = "Best fitted drugs",addlegend = F)
# 
# plot_ave_roc(annotated_pred_df = PRISM_binary_PPIold_data %>% 
#                filter(drug %in% prism_bad_drug_list_200),
#              title = "Worst fitted drugs",addlegend = F)
# 
# plot_ave_roc(annotated_pred_df = PRISM_binary_PPIold_data %>% 
