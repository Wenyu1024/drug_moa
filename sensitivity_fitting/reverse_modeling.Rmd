...
train_and_pred <- function(df){
  ...
  ces_columns <- str_subset(string = colnames(df$training_data[[1]]), pattern = "_demeter")
...
res= perf_ceres %>% left_join(perf_demeter2,by = c("id", ".metric")) %>% left_join(perf_ces,by = c("id", ".metric"))
return(res)
}
