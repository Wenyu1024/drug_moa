...
get_target_pred_accuracy_batch <- 
  function(dataset, id_list= NULL, estimation_method= "fold_cv",target_source){
    ...
    if (estimation_method== "fold_cv") {
      source('/projappl/project_2003466/drug_moa/supervised_target_pred/return_acc_estimate_5fold_cv.R')
    } 
    if (estimation_method== "loocv") {
      source('/projappl/project_2003466/drug_moa/supervised_target_pred/return_acc_estimate_loocv.R')
    } 
