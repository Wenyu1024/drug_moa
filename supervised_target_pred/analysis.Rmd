...
# (ABONDONED! This does not work!)combining target space with sensitivity space and then use them for similarity calculation
...
pairwise.wilcox.test(g = figdata1$method, x = figdata1$value, paired= T,p.adjust.method = "BH")
pairwise.wilcox.test(g = figdata2$method, x = figdata2$value, paired= T,p.adjust.method = "BH")
pairwise.wilcox.test(g = figdata3$method, x = figdata3$value, paired= T,p.adjust.method = "BH")
pairwise.wilcox.test(g = figdata4$method, x = figdata4$value, paired= T,p.adjust.method = "BH")

fig <- figdata1 %>% mutate(Dataset= "CTRP", TargetType= "DRUGBANK") %>% 
  bind_rows(figdata2 %>% mutate(Dataset= "CTRP", TargetType= "DTC")) %>% 
  bind_rows(figdata3 %>% mutate(Dataset= "GDSC", TargetType= "DRUGBANK")) %>% 
  bind_rows(figdata4 %>% mutate(Dataset= "GDSC", TargetType= "DTC")) %>%
  ggplot(
    mapping = aes(
  ...
  mutate(target_or_activity_of_compound.x = 
           recode(target_or_activity_of_compound.x,
                  "inhibitor of BCL2, BCL-xL, and BCL-W" = 
                    "inhibitor of BCL family",
                  "inhibitor of bromodomain (BRD) and extra-C terminal domain (BET) proteins" = "others",
                  "inhibitor of HDAC1, HDAC2, HDAC3, HDAC6, and HDAC8" = "inhibitor of HDAC family",
                  "inhibitor of nicotinamide phosphoribosyltransferase" = "others",
                  "inhibitor of polo-like kinase 1 (PLK1)" = "inhibitor of polo-like kinase I",
                  # "inhibitor of topoisomerase II" = "others",
                  # "inhibitor of BRAF" = "others"
                  ))%>% 
  mutate(drug_pair_name= str_c(cpd_name.x, cpd_name.y,sep = "  ")) %>% 
  mutate(drug_pair_name = as.factor(drug_pair_name) ) %>%
  mutate(drug_pair_name = fct_reorder(drug_pair_name,ess))%>% 
...
ybar <- figdata %>% 
  ...
  coord_flip()+
    # theme(legend.position="none")+
  guides(fill= guide_legend(nrow = 8,title.position = "top"))+
  labs(fill = "Drug mechanism")

# p1 <- cowplot::insert_xaxis_grob(pmain, xbox, grid::unit(0.1, "null"), position = "bottom")
p2 <- cowplot::insert_yaxis_grob(pmain, ybar+theme(legend.position="none"), grid::unit(0.05, "null"), position = "right")
...
# cowplot::ggdraw(p2)
cowplot::plot_grid(p2, legend, rel_heights = c(3, 1.5),nrow = 2)
```

```{r, fig.width=4,fig.height=7}
ybar+
  theme(legend.position="top")+
  guides(fill= guide_legend(nrow = 10,title.position = "top"))+
  labs(fill = "Drug mechanism")
```
...
pairwise.wilcox.test(g = figdata$method, x = figdata$value, paired= T,p.adjust.method = "BH")
# pmain <- figdata %>% 
#   ggplot(mapping = aes(x=  drug_pair_name,y = value, color= method) ) +
#   geom_point()+
#   geom_line(aes(group= method))+
...
kruskal.test(method~value,data= figdata)
pairwise.wilcox.test(g = figdata$method, x = figdata$value, paired= T,p.adjust.method = "BH")
...
p1 <- cowplot::insert_xaxis_grob(pmain, xbox, grid::unit(0.2, "null"), position = "bottom")
```
