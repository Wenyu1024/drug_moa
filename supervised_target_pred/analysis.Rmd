...
acc_df$facet = factor(acc_df$TargetType, 
...
stat.test <- sig_res1 %>%
  ...
  mutate(y.position= case_when(
    TargetType== "Primary"~0.8,
    TargetType== "Kinome-wide Binarized"~0.8,
    TargetType== "Kinome-wide Continues"~0.3,
    )) 
# for unknown reason stat_pvalue_manual function is not working properly. I will use stat_compare_mean and then manually adjust significance level.
...
fig <- acc_df %>% 
  ...
  ggpubr::stat_compare_means(
    ...
    method = "t.test",ref.group = "Ess-sig")+
  stat_summary(
...
ggsave(fig, filename = "~/cluster_wrk/drug_moa/supervised_target_pred/fig_res/fig 3A.pdf",width = 3.5,height = 8)
```
...
fig
ggsave(fig, filename = "~/cluster_wrk/drug_moa/supervised_target_pred/fig_res/supplementary figure1.pdf",width = 3.5,height = 5)
ggsave(fig, filename = "~/cluster_wrk/drug_moa/supervised_target_pred/fig_res/supplementary figure1.png",width = 3.5,height = 5,dpi = 500)
```
...
fig <- cowplot::plot_grid(p2, legend, rel_heights = c(3, 1.5),nrow = 2)
```
...
# cowplot::ggdraw(p2)
fig
ggsave(fig, filename = "~/cluster_wrk/drug_moa/supervised_target_pred/fig_res/fig 3B.pdf",width = 4,height = 8)
```
...
GDSC result does not look good
