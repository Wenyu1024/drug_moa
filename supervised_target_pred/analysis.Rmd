...
# combining target space with sensitivity space and then use similiairtiy
```
...
pairlevel_comparison_ctrp_drugbank <- cor_mat_to_drugpair_longtibble1(tar_ctrp_cor_mat_drugbank,metric="rank_percentage") %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble1(sen_ctrp_cor_mat,metric= "rank_percentage") %>%
      rename(sen =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpess_drugbank_predictions_cormat,metric= "rank_percentage") %>%
      rename(ess =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpexp_drugbank_predictions_cormat,metric= "rank_percentage") %>% rename(exp= value),
    by= c("drug1", "drug2")
    ) %>%   
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpextended_drugbank_predictions_cormat,metric= "rank_percentage") %>%
      rename(extended= value),
        by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpmaccs_drugbank_predictions_cormat,metric= "rank_percentage") %>%
      rename(maccs= value),
        by= c("drug1", "drug2")
    ) %>% 
...
pairlevel_comparison_ctrp_dtc  <- cor_mat_to_drugpair_longtibble(tar_ctrp_cor_mat_dtc,metric="rank_percentage") %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(sen_ctrp_cor_mat,metric= "rank_percentage") %>%
      rename(sen =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpess_dtc_predictions_cormat,metric= "rank_percentage") %>%
      rename(ess =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpexp_dtc_predictions_cormat,metric= "rank_percentage") %>% rename(exp= value),
    by= c("drug1", "drug2")
    ) %>%   
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpextended_dtc_predictions_cormat,metric= "rank_percentage") %>%
      rename(extended= value),
        by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(ctrpmaccs_dtc_predictions_cormat,metric= "rank_percentage") %>%
      rename(maccs= value),
        by= c("drug1", "drug2")
    ) %>% 
  filter(value==1) %>%
...
```{r}
pairlevel_comparison_gdsc_drugbank <- cor_mat_to_drugpair_longtibble(tar_gdsc_cor_mat_drugbank,metric="rank_percentage") %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(sen_gdsc_cor_mat,metric= "rank_percentage") %>%
      rename(sen =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscess_drugbank_predictions_cormat,metric= "rank_percentage") %>%
      rename(ess =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscexp_drugbank_predictions_cormat,metric= "rank_percentage") %>% rename(exp= value),
    by= c("drug1", "drug2")
    ) %>%   
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscextended_drugbank_predictions_cormat,metric= "rank_percentage") %>%
      rename(extended= value),
        by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscmaccs_drugbank_predictions_cormat,metric= "rank_percentage") %>%
      rename(maccs= value),
        by= c("drug1", "drug2")
    ) %>% 
...
pairlevel_comparison_gdsc_dtc  <- cor_mat_to_drugpair_longtibble(tar_gdsc_cor_mat_dtc,metric="rank_percentage") %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(sen_gdsc_cor_mat,metric= "rank_percentage") %>%
      rename(sen =value),
    by= c("drug1", "drug2")
  ...
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscess_dtc_predictions_cormat,metric= "rank_percentage") %>%
      rename(ess =value),
    by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscexp_dtc_predictions_cormat,metric= "rank_percentage") %>% rename(exp= value),
    by= c("drug1", "drug2")
    ) %>%   
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscextended_dtc_predictions_cormat,metric= "rank_percentage") %>%
      rename(extended= value),
        by= c("drug1", "drug2")
    ) %>% 
  inner_join(
    cor_mat_to_drugpair_longtibble(gdscmaccs_dtc_predictions_cormat,metric= "rank_percentage") %>%
      rename(maccs= value),
        by= c("drug1", "drug2")
    ) %>% 
  mutate_at(c("drug1","drug2"), as.integer) %>% 
