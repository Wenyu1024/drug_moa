...
overall_plot <- ggplot(data, aes(x, y)) + geom_line() +
  ...
  geom_vline(xintercept = -1.710127e-03, lty= 2, color= "black")+
  geom_vline(xintercept = 0.0823, lty= 2, color= "red")+
  theme_classic()+
  xlab("Pearson correlation")+
  ylab("")+
  ggtitle("Overall, P-value = 0.019")
...
tmp1 <- drugspecfic_sup %>% filter(score_type==1) %>% filter(threshold_prop== 0.25) %>% 
  arrange(p_perm) %>% 
  inner_join(all_sup[[3]][[1]] %>% select(drug_id, p_adj)) %>% 
  filter(p_adj<0.25)

dist_plot= map(1:7, function(i){
  dens <- density(tmp1$cor_null[[i]])
  data <- tibble(x = dens$x, y = dens$y) %>% 
    mutate(variable = case_when(
      (x > (tmp1$corvalue[i]) ) ~ "On",
      TRUE ~ "off"))
  if(round(tmp1$p_adj[i],digits = 2)==0){
          title= paste0(tmp1$drug_name[i],", ","FDR < 0.01")
  } 
  if(round(tmp1$p_adj[i],digits = 2)!=0){
      title= paste0(tmp1$drug_name[i]," ","FDR = ",round(tmp1$p_adj[i],digits = 2))
  }   

  ggplot(data, aes(x, y)) + geom_line() +
    geom_area(data = filter(data, variable == 'On'), fill = 'grey') +
    geom_vline(xintercept = tmp1$mean_perm[i], lty= 2, color= "black")+
    geom_vline(xintercept = tmp1$corvalue[i], lty= 2, color= "red")+
    theme_classic()+
    xlab("Pearson correlation")+
    ylab("")+
    ggtitle(label = title)
...
ggarrange(overall_plot,dist_plot[[1]],dist_plot[[2]],dist_plot[[3]],dist_plot[[4]],
          dist_plot[[5]],dist_plot[[6]],dist_plot[[7]],
...
tmp %>%   
  ...
  geom_point()+
  geom_text(data=tmp %>% slice(1:3), color="red")+
  xlab("PISA score")+
...
tmp %>%   
  ...
  ylab("Supervised predicion score")+
  theme_classic()  +
  ggtitle("Lanatoside-C")

tmp <- PISA_summary_augmented %>% 
  filter(drug_name=="BMS-387032") %>% 
  arrange(desc(score1)) %>% 
  drop_na() %>% 
  arrange(desc(sup_pred_score))
...
tmp %>%   
  ...
  geom_point()+
  geom_text(data=tmp %>% slice(1:3), color="red")+
  geom_text(data=tmp %>%  arrange(desc(score1)) %>%  slice(1:2), color="red")+
  xlab("PISA score")+
  ylab("Supervised predicion score")+
  theme_classic()  +
  ggtitle("EVP4593")
...
# <!-- https://www.medchemexpress.com/narasin.html -->

tmp <- PISA_summary_augmented %>% 
  filter(drug_name=="Narasin") %>% 
  arrange(desc(score1)) %>% 
  drop_na() %>% 
  arrange(desc(sup_pred_score))
...
tmp %>%   
  ...
  ylab("Supervised predicion score")+
  theme_classic()  +
  ggtitle("Oligomycin-A")
```

```{r}
tmp <- PISA_summary_augmented %>% 
  filter(drug_name=="JIB04") %>% 
  arrange(desc(score1)) %>% 
  drop_na() %>% 
  arrange(desc(sup_pred_score))
...
```{r}
noncancerdrug_PPI_pred %>% 
  ggplot(aes(x=degree, y=abs(unsup_pred_score))) +
